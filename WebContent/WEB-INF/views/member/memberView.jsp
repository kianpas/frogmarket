...
<section class="member-container">
	<div class="my-container">
		...
		<div class="my-profile-view">
			<div class="my-profile-icon">
				<div class="my-icon-img">
					<c:choose>
						<c:when test="${loginMember.icon == null}">
							<img src="<c:url value='/img/icon1.jpg'/>" alt="">
						</c:when>
						<c:otherwise>
							<img src="<c:url value='/img/${loginMember.icon}'/>" alt="">
						</c:otherwise>
					</c:choose>
				</div>
				<div class="my-profile-nick">
					<h3>${loginMember.nickId}</h3>
				</div>
			</div>
			<div class="my-mini-container">
				<div class="my-profile-good">
					<div class="heart">
						<img
							src="<%= request.getContextPath() %>/img/free-icon-hearts-138533.png"
							alt="">
					</div>
					<h3>${loginMember.goodScore}</h3>
				</div>
				<div class="my-profile-market-count">
					<h3>
						아이디 : @${loginMember.memberId}</h3>
				</div>
			</div>
		</div>
		...
		<div class="my-profile-menu">
			...
			<h2>상품</h2>
			<p onclick="noticeList();" style="cursor: pointer; color: red;">댓글
				알림</p>
			...
			<c:if test="${loginMember.memberRole eq MemberService.ADMIN_ROLE}">
			<h2>관리자</h2>
